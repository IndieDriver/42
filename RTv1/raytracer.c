...
double	get_contact(t_map *map, t_vec ray)
{
	...
	b = 2 * ray.x * (map->scene.pos.x - s.pos.x) + 2 * ray.y *
		(map->scene.pos.y - s.pos.y) + 2 * ray.z *
		(map->scene.pos.z - s.pos.z);
...
void	ray(t_map *map, int x, int y)
{
	...
	map->scene.rayvec.x = (double)(x - (map->scene.w / 2.0));
	map->scene.rayvec.y = (double)(y - (map->scene.h / 2.0));
	map->scene.rayvec.z = -(map->scene.w / 2 * tan((30.0 / 2.0) * M_PI / 180.0));
	normalize_vec(&(map->scene.rayvec));
	tmp = get_contact(map, map->scene.raydir, map->scene.rayvec);
	if (tmp > 0.0)
