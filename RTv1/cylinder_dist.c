...
/*   Updated: 2016/03/23 16:12:29 by amathias         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	getdist_cyl(t_cylinder *s, t_vec ray, t_vec opos, int normal)
{
	double	a;
	double	b;
	double	c;
	...
	a = vec_dot(ray, ray) - (vec_dot(ray, s->axis) * vec_dot(ray, s->axis));
	b = 2.0 * (vec_dot(ray, vec_sub(opos, s->pos)) - (vec_dot(ray, s->axis)
				* vec_dot(vec_sub(opos, s->pos), s->axis)));
