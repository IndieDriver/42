...
/*   Updated: 2016/01/27 12:15:25 by amathias         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	bresenham(t_map *map, t_pos from, t_pos to, t_line value)
{
	value.err = 2 * (value.dy - value.dx);
	while ((from.x <= to.x && value.sx == 1)
			|| (from.x >= to.x && value.sx == -1))
	{
		draw_pixel_to_image(map, from.x, from.y, from.color);
		if (value.err >= 0)
		{
			from.y += value.sy;
			value.err += 2 * (value.dy - value.dx);
...
void	bresenham_inverse(t_map *map, t_pos from, t_pos to, t_line value)
{
